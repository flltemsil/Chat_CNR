...
const target = `            <div className="flex items-center gap-1.5 relative z-50">
              {user?.role === 'admin' && (
                <button
          ...
          >
            <div className="max-w-4xl mx-auto w-full">
              {activeSession ? (t-zinc-500 hover:text-zinc-900"}\`}
                title={t.settings}
              >
                <Settings size={16} />
              </button>
...
const replacement = `            <div className="flex items-center gap-1.5 relative z-50">
              {user?.role === 'admin' && (
                <button
                  onClick={() => setIsAdminPanelOpen(true)}
                  className={\`p-2 rounded-lg transition-all border \${theme === "dark" ? "bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white" : "bg-white border-zinc-200 text-zinc-500 hover:text-zinc-900"}\`}
