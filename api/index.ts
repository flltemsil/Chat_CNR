...
app.post("/api/generate-image", async (req, res) => {
  ...
  try {
    if (userApiKey && String(userApiKey).trim().length > 10) {
      try {
        const imageBase64 = await generateWithKey(userApiKey);
    ...
    let lastError: any = null;
    let successImage: string | null = null;

    for (let i = 0; i < apiKeys.length; i++) {
       try {
         successImage = await generateWithKey(apiKeys[i]);
         break;
       } catch (err: any) {
         lastError = err;
       }
    ...
    if (successImage) {
      return res.json({ imageBase64: successImage });
    } else {
       throw lastError;
    }

  } catch (error: any) {
    const errorMsg = String(error.message || "Image generation error");
