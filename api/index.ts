...
app.post("/api/generate-image", async (req, res) => {
  ...
  const generateWithKey = async (key: string) => {
    ...
    // We must use interactions.create for image generation
  ...
  try {
    if (userApiKey && String(userApiKey).trim().length > 10) {
      try {
        const imageUrl = await generateWithKey(userApiKey);
        return res.json({ imageUrl });
      } catch (err: any) {
        console.warn("User provided key failed for image generation:", err.message);
      }
    }
    ...
    for (let i = 0; i < apiKeys.length; i++) {
      try {
        console.log(`Trying system key ${i + 1}/${apiKeys.length} for image generation...`);
        successUrl = await generateWithKey(apiKeys[i]);
        break;
      } catch (error: any) {
        lastError = error;
        console.warn(`System Key ${i + 1} failed for image generation:`, error.message);
      }
    }

    if (successUrl) {
      return res.json({ imageUrl: successUrl });
    } else {
      throw lastError;
    }

  } catch (error: any) {
    const errorMsg = String(error.message || "Görsel üretim hatası");
